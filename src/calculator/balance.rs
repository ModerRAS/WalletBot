...
impl BalanceCalculator {
    ...
    pub async fn update_from_manual_total(
        ...
        // 更新钱包余额
        self.db.update_wallet_balance(wallet_name, total_amount).await?;
    ...
    pub async fn smart_calculate_balance(
        ...
        match total_amount {
            Some(total) => {
                // 如果有总额，直接使用总额更新
                self.update_from_manual_total(wallet_name, total, message_id, chat_id).await
            }
            None => {
                // 如果没有总额，基于交易计算
                let wallet = self.db.get_or_create_wallet(wallet_name).await?;
                ...
                // 更新钱包余额
                self.db.update_wallet_balance(wallet_name, new_balance).await?;
