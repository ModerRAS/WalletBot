...
impl BalanceCalculator {
    ...
    pub async fn update_from_manual_total(
        ...
        // 更新钱包余额
        self.db.update_wallet_balance(chat_id, wallet_name, total_amount).await?;
        
        info!("Updated balance from manual edit: {} {} -> {}", 
    ...
    pub async fn smart_calculate_balance(
        ...
        match total_amount {
            Some(total) => {
                // 如果有总额，直接使用总额更新
                self.update_from_manual_total(chat_id, wallet_name, total, message_id).await
            }
            None => {
                // 如果没有总额，基于交易计算
                let wallet = self.db.get_or_create_wallet(chat_id, wallet_name).await?;
                ...
                let new_balance = self.calculate_transaction_balance(
                    chat_id,
                    wallet_name,
                    transaction_type,
                    amount,
                ...
                // 更新钱包余额
                self.db.update_wallet_balance(chat_id, wallet_name, new_balance).await?;
    ...
    pub async fn get_latest_balance(
        ...
        self.db.get_latest_balance(chat_id, wallet_name, month, year).await
    }
    ...
    pub async fn create_balance_adjustment(
        ...
        info!("Creating balance adjustment for {}: {} -> {} ({})", 
