...
impl Commands {
    ...
    async fn handle_reprocess(&self, bot: &Bot, message: &Message) -> Result<(), RequestError> {
        ...
        if let Some(reply_to) = message.reply_to_message() {
            ...
            bot.send_message(message.chat.id, "Message reprocessed successfully").await?;
        } else {
            bot.send_message(message.chat.id, "Please reply to a message to reprocess it").await?;
        }
